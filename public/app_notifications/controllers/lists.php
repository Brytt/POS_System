    ...
    function Init(){
      // if($this->engine->validatePostForm($this->microtime)){  
        ...
        if(isset($this->fdsearch)){ 
          $query = "SELECT * FROM app_notifications WHERE NOTI_RECEIVER = ".$this->sql->Param('a')." AND NOTI_TYPE LIKE ".$this->sql->Param('a');
          $input =array($actorcode, "%".$this->fdsearch."%"); 
        }else{ 
          $query = "SELECT * FROM app_notifications WHERE NOTI_RECEIVER = ".$this->sql->Param('a');
